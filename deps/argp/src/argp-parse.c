...
static const struct argp_option argp_default_options[] = {
    ...
    {"HANG", OPT_HANG, N_("SECS"), OPTION_ARG_OPTIONAL | OPTION_HIDDEN, N_("Hang for SECS seconds (default 3600)"), 0},
    {0, 0, 0, 0, 0, 0}};
...
static struct group *convert_options(const struct argp *argp, struct group *parent, unsigned parent_index,
                ...
                /* A real option (not just documentation).  */
                {
                    ...
                    /* OPT can be used as a long option.  */
                    {
                        ...
                        cvt->long_end->has_arg =
                            (real->arg ? (real->flags & OPTION_ARG_OPTIONAL ? optional_argument : required_argument)
                                       : no_argument);
                        cvt->long_end->flag = 0;
...
static error_t parser_init(struct parser *parser, const struct argp *argp, int argc, char **argv, int flags,
                           void *input) {
    error_t err = 0;
    struct group *group;
...
static error_t parser_parse_opt(struct parser *parser, int opt, char *val) {
        ...
        if (short_index)
            for (group = parser->groups; group < parser->egroup; group++)
                if (group->short_end > short_index) {
                    err = group_parse(group, &parser->state, opt, parser->opt_data.optarg);
                    break;
                }
    } else
        /* A long option.  We use shifts instead of masking for extracting
           the user value in order to preserve the sign.  */
        err = group_parse(&parser->groups[group_key - 1], &parser->state, (opt << GROUP_BITS) >> GROUP_BITS,
    ...
    /* At least currently, an option not recognized is an error in the
       ...
       with each option.  */
    {
        static const char bad_key_err[] = N_("(PROGRAM ERROR) Option should have been recognized!?");
        if (group_key == 0)
            __argp_error(&parser->state, "-%c: %s", opt, dgettext(parser->argp->argp_domain, bad_key_err));
        else {
            struct option *long_opt = parser->long_opts;
            while (long_opt->val != opt && long_opt->name)
                long_opt++;
            __argp_error(&parser->state, "--%s: %s", long_opt->name ? long_opt->name : "???",
...
static error_t parser_parse_next(struct parser *parser, int *arg_ebadkey) {
    ...
    /* Give getopt a chance to parse this.  */
    {
        ...
        if (parser->state.flags & ARGP_LONG_ONLY)
            opt = _getopt_long_only_r(parser->state.argc, parser->state.argv, parser->short_opts, parser->long_opts, 0,
                                      &parser->opt_data);
        else
            opt = _getopt_long_r(parser->state.argc, parser->state.argv, parser->short_opts, parser->long_opts, 0,
                                 &parser->opt_data);
        ...
        {
            parser->try_getopt = 0;
            if (parser->state.next > 1 && strcmp(parser->state.argv[parser->state.next - 1], QUOTE) == 0)
    ...
    if (opt == KEY_END) {
        ...
        if (parser->state.next >= parser->state.argc || (parser->state.flags & ARGP_NO_ARGS))
