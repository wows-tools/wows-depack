...
%.*s: ARGP_HELP_FMT parameter requires a value"),
                ...
                if (u == nuparam_names)
                    __argp_failure(state, 0, 0, dgettext(state == NULL ? NULL : state->root_argp->argp_domain, "\
...
static int hol_entry_short_iterate(const struct hol_entry *entry,
                                   int (*func)(const struct argp_option *opt, const struct argp_option *real,
...
static inline int __attribute__((always_inline)) hol_entry_long_iterate(const struct hol_entry *entry,
...
static inline int until_short(const struct argp_option *opt, const struct argp_option *real, const char *domain,
                              void *cookie) {
...
/* If the option REAL has an argument, we print it in using the printf
   ...
   optional argument.  */
static void arg(const struct argp_option *real, const char *req_fmt, const char *opt_fmt, const char *domain,
...
static void hol_entry_help(struct hol_entry *entry, const struct argp_state *state, argp_fmtstream_t stream,
                           struct hol_help_state *hhstate) {
    unsigned num;
    ...
    if (pest.first) {
        ...
        const char *tstr = real->doc ? dgettext(state == NULL ? NULL : state->root_argp->argp_domain, real->doc) : 0;
...
static int add_argless_short_opt(const struct argp_option *opt, const struct argp_option *real, const char *domain,
                                 void *cookie) {
...
static int usage_argful_short_opt(const struct argp_option *opt, const struct argp_option *real, const char *domain,
                                  void *cookie) {
...
/* Output a usage entry for the long option opt to the stream pointed at by
   COOKIE.  */
static int usage_long_opt(const struct argp_option *opt, const struct argp_option *real, const char *domain,
                          void *cookie) {
    argp_fmtstream_t stream = cookie;
    const char *arg = opt->arg;
    int flags = opt->flags | real->flags;
...
static int argp_args_usage(const struct argp *argp, const struct argp_state *state, char **levels, int advance,
                           argp_fmtstream_t stream) {
...
static int argp_doc(const struct argp *argp, const struct argp_state *state, int post, int pre_blank, int first_only,
                    argp_fmtstream_t stream) {
    const char *text;
    const char *inp_text;
    void *input = 0;
...
#ifdef _LIBC
            __fxprintf(stream, "%s", state ? state->name : __argp_short_program_name());
#else
            fputs_unlocked(state ? state->name : __argp_short_program_name(), stream);
#endif
...
#ifdef _LIBC
                __fxprintf(stream, ": %s", __strerror_r(errnum, buf, sizeof(buf)));
#else
                putc_unlocked(':', stream);
                putc_unlocked(' ', stream);
